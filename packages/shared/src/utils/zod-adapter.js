 ...
 */
export function zodToResult(zodResult, options = {}) {
    ...
    const validationIssues = options.errorMap
    ...
    const warnings = validationIssues.filter(issue => issue.severity === "warning" || issue.severity === "info");
    ...
    if (warnings.length > 0) {
        ...
        return createSuccessWithWarnings(zodResult.data, warnings);
    }
 ...
 */
function getSuggestionForZodError(error) {
    switch (error.code) {
        case "invalid_type":
            return `Attendu: ${error.expected}, reçu: ${error.received}`;
        case "too_small":
            return `La valeur doit être ${error.type === "string" ? "plus longue" : "plus grande"} (minimum: ${error.minimum})`;
        case "too_big":
            return `La valeur doit être ${error.type === "string" ? "plus courte" : "plus petite"} (maximum: ${error.maximum})`;
        case "invalid_string":
